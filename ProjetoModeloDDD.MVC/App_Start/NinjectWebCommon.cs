...
namespace ProjetoModeloDDD.MVC.App_Start
{
    ...
    public static class NinjectWebCommon 
    {
        ...
        private static void RegisterServices(IKernel kernel)
        {
            ...
            kernel.Bind<IAppServiceDispositivo>().To<AppServiceDispositivo>();

            kernel.Bind(typeof(IServiceBase<>)).To(typeof(ServiceBase<>));
            kernel.Bind<IServiceUsuario>().To<ServiceUsuario>();
            kernel.Bind<IServiceDispositivo>().To<ServiceDispositivo>();


            kernel.Bind(typeof(IRepositoryBase<>)).To(typeof(RepositoryBase<>));
            kernel.Bind<IRepositoryUsuario>().To<RepositoryUsuario>();
            kernel.Bind<IRepositoryDispositivo>().To<RepositoryDispositivo>();
        }        
    }
}
